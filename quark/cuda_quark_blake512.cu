...
//		else
//			quark_blake512_gpu_hash_64 << <grid, block, 0, streamk[thr_id+MAX_GPUS] >> >(threads, (uint64_t*)d_outputHash, order);
	}
...
//		quark_blake512_gpu_hash_80 << <grid, block, 0, streamk[thr_id] >> >(threads, startNounce, d_outputHash);
	}
}
...
//		cudaMemcpy(c_PaddedMessage80, message, sizeof(message), cudaMemcpyHostToDevice);
		cudaMemcpyToSymbolAsync(c_PaddedMessage80, message, sizeof(message), 0, cudaMemcpyHostToDevice, 0);
//		cudaMemcpyToSymbolAsync(c_PaddedMessage80, message, sizeof(message), 0, cudaMemcpyHostToDevice, streamk[thr_id]);
	}
	CUDA_LOG_ERROR();
}
