...
__global__
void quark_keccak512_gpu_hash_64_v30(uint32_t threads, uint64_t *g_hash, int *order)
{
	if (*order) return;
	uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads)
...
//	else
//		quark_keccak512_gpu_hash_64 << <grid, block, 0, streamk[thr_id + MAX_GPUS] >> >(threads, (uint64_t*)d_hash, order);
//	else
//		quark_keccak512_gpu_hash_64_v30 << <grid, block, 0, streamk[thr_id] >> >(threads, (uint64_t*)d_hash, order);
...
void quark_keccak512_cpu_init(int thr_id, uint32_t threads)
{
	...
	cudaMemcpyToSymbol(d_keccak_round_constants, host_keccak_round_constants, sizeof(host_keccak_round_constants), 0, cudaMemcpyHostToDevice);
//	cudaMemcpy(d_keccak_round_constants, host_keccak_round_constants,
//		sizeof(host_keccak_round_constants), cudaMemcpyHostToDevice);
