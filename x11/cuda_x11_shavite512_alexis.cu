...
void x11_shavite512_gpu_hash_64_alexis(const uint32_t threads, uint64_t *g_hash, volatile int *order)
{
#ifdef A1MIN3R_MOD
	if (*order) { return; }
#endif
...
__host__
void x11_shavite512_cpu_hash_64_alexis(int thr_id, uint32_t threads, uint32_t *d_hash, volatile int *order)
{
	dim3 grid((threads + TPB-1)/TPB);
	dim3 block(TPB);

	// note: 128 threads minimum are required to init the shared memory array
	x11_shavite512_gpu_hash_64_alexis << <grid, block >> >(threads, (uint64_t*)d_hash, (volatile int*)order);
//	if (thr_id < MAX_GPUS)
//		x11_shavite512_gpu_hash_64_alexis << <grid, block, 0, streamk[thr_id] >> >(threads, (uint64_t*)d_hash, order);
//	else
//		x11_shavite512_gpu_hash_64_alexis << <grid, block, 0, streamk[thr_id + MAX_GPUS] >> >(threads, (uint64_t*)d_hash, order);
}
